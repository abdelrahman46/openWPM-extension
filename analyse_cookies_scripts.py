...
def get_overall_frequency_of_third_party_cookies ():
	...
	third_party_cookie_domains = set()
	...
	cookie_count_list = []
	query = "SELECT count(id) FROM javascript_cookies WHERE host LIKE '%{HOST_DOMAIN}%'"
	tmp_domain = "{HOST_DOMAIN}"
	for domain in third_party_cookie_domains:
		query = query.replace(tmp_domain, domain)
		cookie_count_list.append([domain, execute_query_and_fetchall(query)[0][0]])
		tmp_domain = domain
...
def get_expiry_dates_statistics_for_every_third_party_cookie_host_domain ():
	...
	for cookie in third_party_cookie_count_list:
		expiry_query = "SELECT expiry FROM javascript_cookies WHERE host LIKE '%" + cookie[0] + "%'"
		expiry_dates = execute_query_and_fetchall(expiry_query)
		...
		for expiry_date in expiry_dates:
			expiry_date = expiry_date[0].split(".")[0]	# remove the .000Z part
